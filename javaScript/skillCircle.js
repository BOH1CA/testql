export function renderPieChart(auditDone, auditReceived) {
    const data = [
    ...
    const arc = d3.arc()
        .innerRadius(0) // Set innerRadius to 0 for pie chart
        .outerRadius(radius); // Adjust the outerRadius if necessary

    const arcHover = d3.arc()
        .innerRadius(0) // Also set innerRadius to 0 for hover effect
        .outerRadius(radius * 0.9); // Slightly increase the outerRadius for hover effect
...
function formatXP(bits, decimals = 2) {
